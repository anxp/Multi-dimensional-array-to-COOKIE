    ...
    if(count($array) > 0) {
        foreach ($array as $key => $value) {
            if(!is_array($value)) { //if element of parent array IS NOT array, we work with it like with end-value, and put it in file.
                ...
                $blackhHole = array_pop($keyPath); //delete sub-array index from the end of path-array. why? because we just dig out one level up, we don't need this index, so put it in $blackHole variable, which never used.
            }
        }
    }
    return ($result);
